...
public class JantarFilosofo {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        if (modo == 1) {
            ...
            System.out.println("Por favor, insira um modo válido.");
        }
    ...
    private static void exibirEstatisticas(Filosofo[] filosofos) {
        ...
        for (Filosofo f : filosofos) {
            ...
            System.out.format("║  Filósofo %d comeu %4d vezes         ║%n", f.getNumeroFilosofo(), r);
            total += r;

            if (r < minRefeicoes) { minRefeicoes = r; filosofoMaisHambrento = f.getNumeroFilosofo(); }
            if (r > maxRefeicoes) { maxRefeicoes = r; filosofoMaisSaciado   = f.getNumeroFilosofo(); }
        }
        ...
        System.out.format( "║  Mais saciado:  Filósofo %d (%d×)      ║%n", filosofoMaisSaciado, maxRefeicoes);
        System.out.printf( "║  Mais faminto:  Filósofo %d (%d×)      ║%n", filosofoMaisHambrento, minRefeicoes);

        if (maxRefeicoes > 0 && (double) minRefeicoes / maxRefeicoes < 0.5) {
            System.out.println("║  ⚠ Desbalanceamento detectado!        ║");
        }
